...
// requestMutexAccess envia CMD_MUTEX_REQUEST e, se cair na fila,
// faz polling ativo (CMD_MUTEX_POLL) até ser promovido.
// NUNCA re-envia CMD_MUTEX_REQUEST enquanto já está na fila —
// isso sobrescreveria o score no ZADD com um Lamport maior,
// atrasando o drone eternamente.
func (d *Drone) requestMutexAccess() bool {
	// ── 1. Envia o pedido inicial ─────────────────────────────────────────
	conn, err := net.Dial("tcp", config.ServerHost+config.TCPPort)
	if err != nil {
	...
	if response.Type == messages.EventMutexGrant {
		return true
	}
	if response.Type != messages.EventMutexQueue {
		return false
	}

	// ── 2. Polling ativo — aguarda ser promovido sem re-enfileirar ────────
	fmt.Printf("[Drone #%d] ⏳ Na fila — aguardando promoção (polling)...\n", d.id)
	for {
		time.Sleep(2 * time.Second)

		pConn, err := net.Dial("tcp", config.ServerHost+config.TCPPort)
		if err != nil {
			continue
		}

		pollMsg := d.buildMsg(messages.CommandMutexPoll, map[string]interface{}{"drone_id": d.id})
		pollData, _ := pollMsg.ToJSON()
		pConn.Write(pollData)

		pbuf := make([]byte, 2048)
		pn, _ := pConn.Read(pbuf)
		pConn.Close()

		pResp, err := messages.FromJSON(pbuf[:pn])
		if err != nil {
			continue
		}
		d.receive(pResp)

		if pResp.Type == messages.EventMutexGrant {
			fmt.Printf("[Drone #%d] 🔑 Mutex poll: GRANT! (lamport=%d)\n",
				d.id, pResp.LamportTS)
			return true
		}
	}
}
...
func (d *Drone) startHeartbeatLoop() {
	...
	emergencySent := false
	tick := 0
	mutexInProgress := false // evita sobrepor pedidos de mutex
	...
	for d.status.Battery > 0 {
		...
		// A cada 5 heartbeats tenta acessar recurso compartilhado (mutex Redis)
		if tick%5 == 0 && d.status.Battery > 15 && !mutexInProgress {
			mutexInProgress = true
			go func() {
				fmt.Printf("\n[Drone #%d] 🔑 Tentando acessar recurso compartilhado...\n", d.id)
				granted := d.requestMutexAccess()
				if granted {
					fmt.Printf("[Drone #%d] ✅ Acessando recurso (seção crítica) — segurando por 15s para visualização\n", d.id)
					time.Sleep(3 * time.Second)
					recharge := 100
					d.status.Battery += recharge
					if d.status.Battery > 100 {
						d.status.Battery = 100
					}
					fmt.Printf("[Drone #%d] 🔋 Bateria recarregada +%d%% → %d%%\n", d.id, recharge, d.status.Battery)
					d.releaseMutex()
				}
				mutexInProgress = false
				fmt.Println()
			}()
		}
		...
		time.Sleep(1 * time.Second) // ← reduzido: drones pedem mutex mais rápido → fila acumula
	}
