...
// PollAccess verifica se é a vez do drone sem re-enfileirar.
// Chamado pelo drone enquanto aguarda na fila (polling).
func (rm *ResourceManager) PollAccess(droneID int) bool {
	// Há owner atual?
	owner, _ := rm.redisClient.GetMutexOwner()
	if owner != "" {
		return false
	...
	// Este drone é o topo da fila?
	topID, _, err := rm.redisClient.PeekMutexQueue()
	if err != nil || topID != droneID {
		return false
	}

	// Concede
	if err := rm.redisClient.SetMutexOwner(droneID); err != nil {
		return false
	}
	fmt.Printf("[Mutex] ✅ POLL-GRANT para Drone #%d (chegou sua vez)\n", droneID)
	return true
}
