...
// Subscribe retorna um canal Go que recebe mensagens dos canais Redis.
// Mantém a referência ao PubSub viva e reconecta automaticamente em caso de queda.
func (c *Client) Subscribe(channels ...string) <-chan *redis.Message {
	out := make(chan *redis.Message, 100)

	go func() {
		for {
			sub := c.rdb.Subscribe(c.ctx, channels...)

			// Confirma que a subscription foi aceita pelo Redis
			_, err := sub.Receive(c.ctx)
			if err != nil {
				fmt.Printf("[Redis] ⚠️  Erro ao confirmar subscription: %v — reconectando em 2s...\n", err)
				sub.Close()
				time.Sleep(2 * time.Second)
				continue
			}

			fmt.Printf("[Redis] ✓ Pub/Sub ativo nos canais: %v\n", channels)

			// Repassa mensagens para o canal de saída
			for msg := range sub.Channel() {
				out <- msg
			}

			// Canal fechou — reconecta
			fmt.Println("[Redis] ⚠️  Pub/Sub desconectado — reconectando em 2s...")
			sub.Close()
			time.Sleep(2 * time.Second)
		}
	}()
...
// SetMutexOwner define o detentor do mutex (TTL de 8s — segurança contra crash do drone).
func (c *Client) SetMutexOwner(droneID int) error {
	val := fmt.Sprintf("%d", droneID)
	return c.rdb.Set(c.ctx, KeyMutexOwner, val, 8*time.Second).Err()
}
...
// SetLamportClock persiste o valor atual do relógio de Lamport no Redis.
// Chamado pelo orquestrador a cada receive/tick para manter a chave visível
// no Redis Commander em tempo real.
func (c *Client) SetLamportClock(value uint64) {
	c.rdb.Set(c.ctx, KeyLamportGlobal, value, 0)
}
